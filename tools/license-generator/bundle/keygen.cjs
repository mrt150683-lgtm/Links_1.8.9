...
function generateKeypair() {
  ...
  const privkeyBytes = (0, import_crypto.randomBytes)(32);
  const privkeyHex = bytesToHex2(privkeyBytes);
  const pubkeyBytes = getPublicKey(privkeyBytes);
  const pubkeyHex = bytesToHex2(pubkeyBytes);
  console.log("PRIVATE KEY (hex) \u2014 Keep SECRET, never commit:");
